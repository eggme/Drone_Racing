...
namespace PA_DronePack
{
    public class PA_DroneCamera : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            {
                if (target.rigidBody.interpolation != RigidbodyInterpolation.Interpolate && !GetComponent<Camera>().targetTexture) { target.rigidBody.interpolation = RigidbodyInterpolation.Interpolate; }
            ...
            if (target && fpsPosition && cameraMode == CameraMode.firstPerson && !GetComponent<Camera>().targetTexture)
            {
